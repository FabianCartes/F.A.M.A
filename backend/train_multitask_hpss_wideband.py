"""
backend/train_multitask_hpss_wideband.py
Entrenamiento de MultiTaskBioacousticModel (ResNet34d) con:
1. Frontend HPSS de banda ancha (Wideband HPSS: f_max=14000 Hz, sr=32000 Hz) para retener el chillido armónico de correas (>8kHz).
2. Ventana acústica ajustada a 1.5s sin relleno artificial de ceros para el 87% del dataset.
3. Síntesis aditiva física guiada por ontología mecánica.
4. Calibración estricta de alpha_prior en val_df antes de evaluación ciega en test_df.
"""
...
def main():
    ...
    # Configuración de física acústica de Banda Ancha y Cero Padding
    audio_cfg = AudioConfig(
        target_sr=32000,
        duration_seconds=1.5,   # Duración nativa exacta para el 87% de las muestras (cero padding de silencio)
        n_mels=128,
        n_fft=2048,
        hop_length=256,
        f_min=20.0,
        f_max=14000.0,          # Banda ancha extendida hasta 14 kHz para capturar armónicos de fricción y correas
    )

    # 1. Frontend HPSS 3 Canales de Banda Ancha
    ...
    print("\n" + "=" * 60)
    print(f"[Entrenamiento] ResNet34d Wideband HPSS (14kHz, 1.5s) + Síntesis Aditiva ({epochs} épocas)")
    print("=" * 60)
    ...
    # 5. Calibrar alpha_prior óptimo exclusivamente en validación
    print("\n" + "=" * 60)
    print("[Calibración de Alpha Prior en Validación]")
    best_alpha = 0.25
    best_calib_f1 = -1.0
    for alpha_candidate in np.linspace(0.0, 0.5, 11):
        ...
        print(f"  Alpha = {alpha_candidate:.2f} -> Val Macro F1: {cand_f1*100:.2f}%")
        if cand_f1 > best_calib_f1:
            best_calib_f1 = cand_f1
            best_alpha = float(alpha_candidate)

    print(f"  >> Alpha óptimo seleccionado: {best_alpha:.2f} (Val F1: {best_calib_f1*100:.2f}%)")

    # 6. Cargar mejor estado y evaluación ciega en test_df usando alpha óptimo
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS RESNET34D WIDEBAND HPSS (14kHz, 1.5s) EN TEST SET CIEGO:")
    print(f"  >> VALIDACIÓN: Best Val Macro F1 = {best_val_f1*100:.2f}% (Calibrated F1: {best_calib_f1*100:.2f}%)")
    ...
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-resnet34d-multitask-hpss-wideband")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_state_dict, ckpt_dir / "weights.pt")

    receipt_path = Path(\"docs/receipts/fase9_resnet34d_wideband_receipt.json\")
    with open(receipt_path, "w", encoding="utf-8") as f:
        json.dump({
            "model_id": "car-engine-diagnostics-resnet34d-multitask-hpss-wideband",
            "technique": "Wideband HPSS (f_max=14000Hz, duration=1.5s, no-padding) + Additive Mixing",
