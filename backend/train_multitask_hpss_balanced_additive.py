"""
backend/train_multitask_hpss_balanced_additive.py
Entrenamiento de MultiTaskBioacousticModel (ResNet34d) con:
1. Muestreo balanceado uniforme (WeightedRandomSampler) por clase
2. Síntesis Aditiva Física continua (p_synth=0.75) para transformar clases compuestas en flujo infinito
3. Frontend HPSS de 3 canales (Original, Armónico, Percusivo)
4. Cabeza Multi-Task desacoplada (13 clases + 7 subsistemas ortogonales)
"""
...
from training.pipelines.additive_mixing import AdditiveCompoundSampler, build_balanced_class_sampler
...
def main():
    ...
    print(f"\n[Setup] Dispositivo: {device} | Modelo: ResNet34d MultiTask HPSS + Balanced Continuous Additive")
    ...
    # Sampler aditivo continuo con probabilidad 0.75
    ...
    # Sampler balanceado uniforme por clase
    balanced_sampler = build_balanced_class_sampler(train_df, class_col="clase")
    ...
    # Con sampler, shuffle debe ser False
    train_loader = DataLoader(train_dataset, batch_size=32, sampler=balanced_sampler, pin_memory=True)
    ...
    print("\n" + "=" * 60)
    print(f"[Entrenamiento] ResNet34d Balanced Continuous Additive ({epochs} épocas)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS STANDALONE RESNET34d BALANCED CONTINUOUS ADDITIVE EN TEST SET CIEGO:")
    ...
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-resnet34d-multitask-hpss-balanced-additive")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_state_dict, ckpt_dir / "weights.pt")

    receipt_path = Path(\"docs/receipts/fase8_balanced_additive_receipt.json\")
    with open(receipt_path, "w", encoding="utf-8") as f:
        json.dump({
            "model_id": "car-engine-diagnostics-resnet34d-multitask-hpss-balanced-additive",
            "architecture": "resnet34d_multitask_hpss_gem_balanced_additive",
            "technique": "Balanced Weighted Random Sampling + Continuous Additive Synthesis (p=0.75)",
            "validation_metrics": {
