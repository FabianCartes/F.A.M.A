...
Entrenamiento de MultiTaskBioacousticModel con entrada HPSS de 3 canales y síntesis física aditiva
(Physics-Guided Additive Mixing) para multiplicar la representatividad acústica de fallas compuestas.
"""
...
def main():
    ...
    # 2. Sampler de Síntesis Aditiva Física guiada por ontología (solo para train_df)
    additive_sampler = AdditiveCompoundSampler(
        df=train_df,
        target_sr=audio_cfg.target_sr,
        duration_seconds=audio_cfg.duration_seconds,
    )
    print(f"[Additive Synthesis] Sampler inicializado para clases compuestas: {list(additive_sampler.class_to_paths.keys())}")

    # 3. Datasets: train con muestreo aditivo dinámico (p=0.5), val y test limpios
    ...
    # 4. Modelo MultiTask Bioacústico
    ...
    print("\n" + "=" * 60)
    print(f"[Entrenamiento] Iniciando MultiTask HPSS con Síntesis Aditiva ({epochs} épocas)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS STANDALONE CON SÍNTESIS ADITIVA EN TEST SET CIEGO:")
    ...
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-resnet34d-multitask-hpss-additive")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_state_dict, ckpt_dir / "weights.pt")

    receipt_path = Path(\"docs/receipts/fase4_additive_mixing_receipt.json\")
    with open(receipt_path, "w", encoding="utf-8") as f:
        json.dump({
            "model_id": "car-engine-diagnostics-resnet34d-multitask-hpss-additive",
            "technique": "Physics-Guided Additive Mixing (Dynamic Synthesis)",
            "validation_metrics": {
