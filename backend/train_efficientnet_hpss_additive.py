"""
backend/train_efficientnet_hpss_additive.py
Entrenamiento de MultiTaskBioacousticModel (EfficientNet-B0) con entrada HPSS de 3 canales
y síntesis física aditiva (Physics-Guided Additive Mixing) para dotar al segundo backbone
del desacoplamiento espectral y ontológico necesario para el Super-Ensamble.
"""
...
def main():
    ...
    print(f"\n[Setup] Dispositivo: {device} | Modelo: EfficientNet-B0 HPSS MultiTask Additive")
    ...
    # 2. Sampler de Síntesis Aditiva Física (solo para train_df)
    ...
    # 4. Modelo MultiTask con EfficientNet-B0
    ...
    print("\n" + "=" * 60)
    print(f"[Entrenamiento] Iniciando EfficientNet-B0 HPSS MultiTask con Síntesis Aditiva ({epochs} épocas)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS STANDALONE EFFICIENTNET-B0 HPSS ADITIVO EN TEST SET CIEGO:")
    ...
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-efficientnet-b0-multitask-hpss-additive")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_state_dict, ckpt_dir / "weights.pt")

    receipt_path = Path(\"docs/receipts/fase5_efficientnet_additive_receipt.json\")
    with open(receipt_path, "w", encoding="utf-8") as f:
        json.dump({
            "model_id": "car-engine-diagnostics-efficientnet-b0-multitask-hpss-additive",
            "architecture": "efficientnet_b0_multitask_hpss_gem",
            "technique": "Physics-Guided Additive Mixing + HPSS 3ch",
            "validation_metrics": {
