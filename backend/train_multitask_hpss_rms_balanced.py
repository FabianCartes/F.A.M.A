"""
backend/train_multitask_hpss_rms_balanced.py
Entrenamiento de MultiTaskBioacousticModel (ResNet34d) con:
1. Síntesis aditiva física con balanceo de potencia acústica RMS (RMS Power Balancing).
   Resuelve el enmascaramiento acústico donde señales de alto RMS (low_oil, 0.187) ahogaban
   señales de bajo RMS (power_steering, 0.005), eliminando falsos positivos en el atributo steering.
2. Frontend HPSS 3 canales optimizado (f_min=20 Hz, f_max=8000 Hz, 32kHz, 2.0s).
3. Calibración de fiabilidad de atributos en validación.
4. Evaluación ciega bajo Test Guard SHA-256.
"""
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"[Entrenamiento] ResNet34d HPSS + RMS-Balanced Additive Mixing ({epochs} épocas)")
    print("=" * 60)
    ...
    # 5. Calibrar alpha_prior y fiabilidad en validación
    print("\n" + "=" * 60)
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS STANDALONE RESNET34D RMS-BALANCED EN TEST SET CIEGO:")
    ...
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-resnet34d-multitask-hpss-rms-balanced")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_state_dict, ckpt_dir / "weights.pt")

    receipt_path = Path(\"docs/receipts/fase10_rms_balanced_additive_receipt.json\")
    with open(receipt_path, "w", encoding="utf-8") as f:
        json.dump({
            "model_id": "car-engine-diagnostics-resnet34d-multitask-hpss-rms-balanced",
            "technique": "RMS-Balanced Additive Mixing + HPSS 3ch",
