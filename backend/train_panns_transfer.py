...
frontend HPSS de 3 canales y evaluación comparativa / ensamble tri-modelo.
"""
...
def main():
    ...
    print(f"[Setup] Dispositivo: {device} | Clases: {num_classes}")
    ...
    # 1. Instanciar PANNs CNN14 y cargar pesos de AudioSet
    print("\n[PANNs] Instanciando CNN14 con congelamiento de primeras 4 etapas (ConvBlocks 1-4 congelados)...")
    model = PannsCNN14(
        in_chans=3,
    ...
    # Parámetros entrenables únicamente (ConvBlocks 5, 6, FC)
    ...
    print("\n" + "=" * 60)
    print("[Fase 2] Iniciando fine-tuning de PANNs CNN14 (30 épocas)")
    print("=" * 60)
    ...
    test_probs = torch.softmax(all_test_logits, dim=-1).numpy()
    ...
    print("\n" + "=" * 60)
    print("RESULTADOS PANNs CNN14 (STANDALONE) EN TEST SET CIEGO:")
    print(f"  >> Val Macro F1:  {best_val_f1*100:.2f}%")
    ...
    # Guardar Checkpoint
    ckpt_dir = Path("backend/checkpoints/car-engine-diagnostics-panns-cnn14-hpss")
